...
Round 1 (gpt-5-5 current edit against `mcd8604__hunter`):
- `/logs/rounds/0` is a strong win (`38437` vs `1672`) with all 25 ten-round files first place and no traced live losses (248 wins / 2 draw-ish timeout/end traces by final energy). Hunter is a compact speed-5.3 circular/turning mover that fires mostly power-3; its score comes from occasional close p3 hits, not from survival.
- Added `tools/analyze_hunter.py` for quick outcome/range/fire summaries. `tools/offline_gun_eval.py '/logs/rounds/0/sim_*.jsonl'` and a shot-time replay both show circular/averaged prediction far ahead of head-on/linear; circular p3 has very small median error despite a few wall/stop outliers.
- Added a narrow `mcd8604__hunter` profile in `robots/custom/MyTank.java`: early-return circular gun with max-power pressure while energy is safe, a medium compact ~355px orbit (widening only if energy falls), and direct separation below ~300px/370px low-energy. This keeps the proven heavy circular kill speed but avoids the generic Crazy 260px knife-range pull-in that leaked Hunter's p3 bullet damage.
- Recompiled successfully with `javac -cp libs/robocode.jar robots/custom/MyTank.java`. Next teammate: compare `/logs/rounds/1`; if score drops from slower/wider rounds, revert just the Hunter preferred distance/close escape toward the generic Crazy values while keeping circular gun forcing.

Round 2 (gpt-5-5 current edit, Hunter follow-up):
- Reviewed `/logs/rounds/1` after the first `mcd8604__hunter` specialization. It improved slightly over round 0 (`38532` vs `38437`) and produced a clean 250/250 live sweep. Hunter remains a compact speed-5.33 circular/turning bot firing mostly p3; our end energy is high and the specialization's wider ~355px healthy orbit reduced close leakage without slowing kills.
- `tools/offline_gun_eval.py '/logs/rounds/1/sim_*.jsonl'` shows circular and averaged are both excellent on actual Hunter shots, with averaged now marginally lower mean error (`avg ~41.5px`, `circ ~43.0px`). Tiny gun retune only: in `doHunterGun()`, allow the virtual gun to switch from circular to averaged with a 0.75px margin instead of requiring a 3px margin. Movement/power are unchanged to preserve the safe sweep.
- Recompiled successfully with `javac -cp libs/robocode.jar robots/custom/MyTank.java`.

Round 1 (gpt-5-5 current edit against `johan_adriaans__berendbotje`):
- `/logs/rounds/0` is currently a losing aggregate (`24170` vs `17744`) despite our higher bullet damage (`12966` vs `10923`). Trace survival is poor (`tools/analyze_berendbotje.py /logs/rounds/0`: about 59 wins / 185 losses / 6 draws). BerendBotje is a medium/fast turning mover (avg speed ~5.4, ~43% full-speed ticks, avg turn ~0.09-0.10) that fires frequent p2-p3 bullets; our generic bot fought too close (~235px avg) with ~p2 shots and self-depleted while it survived with large energy.
...
Round 3 (gpt-5-5 current edit, BerendBotje follow-up 2):
- Reviewed `/logs/rounds/2`: the round-2 BerendBotje widening/stronger-bullet retune regressed badly (`17966` vs round-1 `21062`; live wins ~54/250 vs ~77/250). Loss traces showed our pure `driveAwayFrom()` close escape often collapsed into low-lateral 130-220px chases, while the wider 475/535/585 lane and p2-ish opening shots still self-depleted against BerendBotje's p2/p3 stream.
- Partially reverted BerendBotje numbers toward the better round-1 profile: preferred range back to ~430/485/540, healthy/mid powers back to faster medium bullets (~1.55-1.85 / ~0.92-1.20), and fire/bullet-hit escape distances slightly reduced from the over-wide round-2 values.
- Changed BerendBotje scan-time close escape from pure radial `driveAwayFrom()` to `driveSampleWallsEscape()` (away + perpendicular) and only triggers direct close handling below ~320/390px; actual `onHitRobot` collision handling remains radial separation to break overlap quickly. Goal: keep lateral velocity while opening range, instead of being chased straight down the bearing line.
- Recompiled successfully with `javac -cp libs/robocode.jar robots/custom/MyTank.java`. If `/logs/rounds/3` regresses, revert only these BerendBotje edits to the previous commit; averaged gun remains best in offline replay (`avg` mean ~70px on round 2 vs head ~79/circ ~89/linear ~98).

Round 4 (gpt-5-5 current edit, BerendBotje follow-up 3):
- Reviewed `/logs/rounds/3`: round-3 code improved aggregate score vs round 2 (`21322`) but still lost badly to `johan_adriaans__berendbotje` (`37321`). Traces show the main regression mode is extreme range collapse: average range only ~171px, ~79% of sampled ticks under 250px, huge ram/collision leakage (our ram column 874, opp 590+89 bonus), and Berend still wins ~190/250 live outcomes. Our shot power averaged ~1.35 while opp drops averaged ~2.0.
- `tools/offline_gun_eval.py '/logs/rounds/3/sim_*.jsonl'` still favors the normal averaged predictor (`avg` mean ~50px, circ/head ~55, linear ~61), so I left `doBerendBotjeGun()`'s averaged-first gun/power profile unchanged.
- Movement-only BerendBotje change in `robots/custom/MyTank.java`: added `driveKiteEscape()` (diagonal-away lane scoring with strong separation plus lateral velocity), use it on Berend detected-fire ticks and bullet hits, trigger close-range kiting earlier (<470 healthy / <520 low), widen nominal Berend orbit to ~525/575/620, and make robot-collision separation stronger. The intent is to break the 50-220px chase/ram loop while preserving averaged fast bullets.
- Recompiled successfully with `javac -cp libs/robocode.jar robots/custom/MyTank.java`. If `/logs/rounds/4` gets worse, revert only this Round-4 movement/kite change; the best historical score against Berend was round 1 (`21062`) and round 3 was close (`21322`) but with far too much close-contact damage.

Round 5 (gpt-5-5 current edit, BerendBotje final):
- Reviewed `/logs/rounds/4`: the round-4 wide/kite movement retune was a clear regression (`17568` vs BerendBotje `30831`, live ~55/250 wins). The best historical aggregate for this opponent was round 3 (`21322`), with round 1 close (`21062`); the wide/kite branch did not increase average range enough and reduced score.
- Reverted the BerendBotje-specific movement/power changes back to the stronger round-1 baseline from commit `a6246c7`: fire-tick `drivePerpendicularEscape`, close radial escape only below 285/360, 430/485/540 preferred range, original conservative averaged-gun power ladder, original hit/collision escapes. Kept one proven round-2 improvement: exclude BerendBotje from the generic hard-to-hit 355px override so its name-gated range is not stolen late.
- Recompiled successfully with `javac -cp libs/robocode.jar robots/custom/MyTank.java`. This should be safer than round 4; future work should avoid extreme wide/kite changes unless local/harness A/B proves it helps.
