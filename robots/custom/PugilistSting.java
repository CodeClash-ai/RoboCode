...
public class PugilistSting {
	...
	int distanceIndex = (int)Math.min(StingWave.DISTANCE_INDEXES - 1, (enemyDistance / (MAX_DISTANCE / StingWave.DISTANCE_INDEXES)));
	...
	StingWave wave = new StingWave(robot);
	...
	wave.bearingDirection = lastEnemyBearingDirection / (double)StingWave.MIDDLE_FACTOR;

	int wallIndex = 0;
	do {
	} while (++wallIndex < (StingWave.WALL_INDEXES) &&
	...
	wave.visits = StingWave.factors[distanceIndex][velocityIndex][lastVelocityIndex]
	    [(int)PUtils.minMax(Math.pow(enemyTimeSinceDeccel++, 0.45) - 1, 0, StingWave.DECCEL_TIME_INDEXES - 1)][wallIndex];

	robot.setTurnGunRightRadians(Utils.normalRelativeAngle(wave.startBearing - robot.getGunHeadingRadians() +
		    wave.bearingDirection * (wave.mostVisited() - StingWave.MIDDLE_FACTOR)));
...
class StingWave extends Condition {
    ...
    public StingWave(AdvancedRobot robot) {
	this.robot = robot;
    }
