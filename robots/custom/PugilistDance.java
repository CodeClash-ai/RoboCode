...
// Source: https://github.com/PEZ/Bots/blob/HEAD/pez/clean/Swiffer.java
...
public class PugilistDance {
	...
	DanceWave.passingWave = null;
    }

    public void onScannedRobot(ScannedRobotEvent e) {
	DanceWave ew = new DanceWave(robot, this);
	...
	ew.bearingDirection = Math.asin(MAX_VELOCITY / ew.bulletVelocity) * direction / (double)DanceWave.MIDDLE_FACTOR;

	ew.visits = DanceWave.factors
	    [distanceIndex = (int)Math.min(DanceWave.DISTANCE_INDEXES - 1, (enemyDistance / (MAX_DISTANCE / DanceWave.DISTANCE_INDEXES)))]
	...
	if (DanceWave.visitsReverse < DanceWave.visitsForward) {
	    direction = -direction;
	}
	DanceWave.visitsForward = DanceWave.visitsReverse = 0;
    }

    public void onHitByBullet(HitByBulletEvent e) {
	DanceWave wave = DanceWave.passingWave;
	if (wave != null) {
    ...
    void updateDirectionStats(DanceWave wave) {
	DanceWave.visitsReverse += wave.smoothedVisits(waveImpactLocation(wave, -1.0, 5));
	DanceWave.visitsForward += wave.smoothedVisits(waveImpactLocation(wave, 1.0, 0));
    }

    Point2D waveImpactLocation(DanceWave wave, double direction, double timeOffset) {
...
class DanceWave extends Condition {
    ...
    public DanceWave(AdvancedRobot robot, PugilistDance dancer) {
	this.robot = robot;
	this.dancer = dancer;
    }
