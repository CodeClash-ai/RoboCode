...
// Source: https://github.com/PEZ/Bots/blob/HEAD/pez/mako/Mako.java
...
class CheckVirtualGunsCondition extends Condition {
    ...
    public CheckVirtualGunsCondition(Bullet bullet, double bearingDelta, double enemyDistance,
        ...
        this.oldBearing = MyTank.absoluteBearing(oldRLocation, oldELocation);
    }
    ...
    public boolean test() {
        ...
        if (bulletDistance > oldRLocation.distance(target.getLocation()) - 10) {
            ...
            if (Math.abs(oldBearingDelta) > 0.05) {
                double impactBearing = MyTank.absoluteBearing(oldRLocation, target.getLocation());
                MyTank.toLocation(impactBearing, bulletDistance, oldRLocation, impactLocation);
                double bearingDiff = MyTank.normalRelativeAngle(impactBearing - oldBearing);
                ...
                for (int i = 0; i < guns.length; i++) {
                    ...
                    MyTank.toLocation(((FactorGun)gun).guessedBearing(oldBearing, oldBearingDelta),
...
class TuningFactor implements Rateable {
    ...
    void updateRating(double rating, double weight) {
        this.rating = MyTank.rollingAvg(this.rating, rating, Math.min(++uses, ratingDepth), weight);
    }
...
abstract class VirtualGun implements Rateable {
    ...
    void updateRating(double rating, double weight) {
        uses++;
        this.rating = MyTank.rollingAvg(this.rating, rating, Math.min(++ratings, VG_RATING_DEPTH), weight);
    }
...
class AngularFactoredGun extends FactorGun {
    ...
    void updateFactor(double factor, double bearingDiff, double weight) {
        this.factor = MyTank.rollingAvg(this.factor, factor, Math.min(++factorUpdates, factorDepth), weight);
    }
...
class BearingOffsetGun extends FactorGun {
    ...
    void updateFactor(double factor, double bearingDiff, double weight) {
        this.factor = MyTank.rollingAvg(this.factor, bearingDiff, Math.min(++factorUpdates, factorDepth), weight);
    }
